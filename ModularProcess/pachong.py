...
async def getStockList(page, listURL):
    ...
    for x in htmlInBS.find_all("a",
                               attrs={
                                #    "class": "ared",
                                    "target": "_blank",
                                    "href"  : re.compile("^gpdm\.asp\?gpdm=(\d{6})")
                               }):
...
async def getStockInfo(oneStock , page , stockURL ):
    ...
    url = stockURL + oneStock[1] + "/lshq.shtml"  # oneStock[1] is the code of this stock
    print(url)
    await page.goto(url)
...
async def main():
    ...
    oneStockInfoList = await getStockInfo(stockCodeList[2], page, individualURL)
