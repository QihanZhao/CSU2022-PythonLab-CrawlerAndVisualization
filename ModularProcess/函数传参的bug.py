...
def storeHistoryData(historyData):
    global conn
    makeConn()
    try:
        with conn.cursor() as cursor:
            for s in historyData:
                # print(s[0])
                cursor.execute(
...
def readHistoryData(historyData):
    global conn
    makeConn()
    try:
        with conn.cursor() as cursor:
...
def doubleClickinList(event):
    ...
    historyData = [(i[0:3] + i[5:7]) for i in rawData]
    ...
    for i in range(len(historyData)):
        treeStockInfo.insert('', 'end', values=historyData[i])
