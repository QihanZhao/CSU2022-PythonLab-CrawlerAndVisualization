...
async def getStcockList():
    ...
    await crawler.antiAntiCrawler(page)  #新页面生成后一律调用此来反反爬

    listURL = "http://www.shdjt.com/flsort.asp?lb=993505"
    stockList = await crawler.getStockList(page, listURL)
...
def doubleClickinList(event):
    ...
    infoTask = loop.create_task(
        crawler.getStockInfo(stockList[i[0]], page,
    ...
    crawler.showOneKLineChart(rawData)
